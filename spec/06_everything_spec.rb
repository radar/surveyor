...
RSpec.describe '05: Everything together' do
  let(:survey) { Surveyor::Survey.new(name: 'ACME Engagement Survey') }
  let(:question_1) { Surveyor::Question.new(title: 'Do you feel supported by your manager?') }
  let(:question_2) { Surveyor::Question.new(title: 'Do you feel like the work you do is meaningful?') }

  let(:response_1) { Surveyor::Response.new(user: "alice@example.com") }
  let(:answer_1_1) { Surveyor::Answer.new(type: 'rating', value: 5, question: question_1) }
  let(:answer_1_2) { Surveyor::Answer.new(type: 'rating', value: 3, question: question_2) }

  let(:response_2) { Surveyor::Response.new(user: "bob@example.com") }
  let(:answer_2_1) { Surveyor::Answer.new(type: 'rating', value: 3, question: question_1) }
  let(:answer_2_2) { Surveyor::Answer.new(type: 'rating', value: 3, question: question_2) }

  let(:response_3) { Surveyor::Response.new(user: "claire@example.com") }
  let(:answer_3_1) { Surveyor::Answer.new(type: 'rating', value: 5, question: question_1) }
